...
def load_dataset(path, split=0.2):
    images = sorted(glob(os.path.join(path, "original_axial", "*.jpg")))
    masks = sorted(glob(os.path.join(path, "bin_mask", "*.jpg")))
...
if __name__ == "__main__":
    ...
    model.compile(loss=dice_loss, optimizer=adam_v2.Adam(lr), metrics=metrics)
